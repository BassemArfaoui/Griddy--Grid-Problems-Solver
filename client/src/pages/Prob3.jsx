...
const NetworkCoverageProblem = () => {
  ...
  const [coverage, setCoverage] = useState(null); // state for coverage
  ...
  const sendNetworkDataToAPI = async () => {
    if (
      ...
      notify("Invalid input: Number of routers must be non-negative numbers.");
      return;
    }
    ...
    try {
      ...
      notify("Your Situation is too complex for a Free Gurobi license.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const resetGrid = () => {
    setNetworkGrid(Array.from({ length: n }, () => Array(n).fill("-")));
    setNumRange5(0);
    setNumRange3(0);
    setBlockedCells([]);
    setLoading(false);
  ...
  const calculateCoverage = () => {
    let coveredCells = 0;
    let totalCells = 0;
    networkGrid.forEach((row) => {
      row.forEach((cell) => {
        if (cell === "C" || cell === "R") coveredCells++;
        if (cell !== "W") totalCells++; 
      });
    });
    return { coveredCells, totalCells };
  };

  const { coveredCells, totalCells } = calculateCoverage();

  return (
    <div className="cont-container pt-5" style={{height:'100vh'}}>
      <div className="main d-flex flex-row gap-5 justify-content-center pt-5">
        <div
        ...
        <div className="d-flex flex-column gap-2 align-items-center">
          <span
            id="length"
            className="length  rounded-3  fw-bold border-3 border-secondary align-self-center"
          ...
          >
            {!loading ? (
              ...
              <span
                className="text-secondary small d-flex justify-content-center align-items-center"
                style={{ padding: "1pt 13px" }}
              >
                Loading ...
              </span>
            )}
